...
A single page app using dvajs, create-react-app and Go. 

The javascript code has been derived from dvajs example: [popular-products](https://github.com/dvajs/dva/tree/master/examples/popular-products)
The code has changed to accommodate eslint, flowtype and other simplifications.

## Running the api server

The api server is written in Go with sqlite as the database. The database is initialized with products data
...
- /api/products
- /api/products/vote/:id

## Proxying API Requests

The proxy field in package.json is to be set accordingly.

```
"proxy": "http://localhost:4000"
```

See: [Proxying API Requests in Development](#proxying-api-requests-in-development)

## Serving build/index.html, build/static

The api server also serves the build directory generated by:
$ npm run build

The server has a catch-all route "/*" needed for single page apps.
...
If you have Go installed you could simply run the api server:
$ go run api.go
